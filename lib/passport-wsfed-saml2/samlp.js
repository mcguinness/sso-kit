...
var Samlp = module.exports = function Samlp (options, saml) {
  ...
  this.options.path = options.path || '/sso/saml2/acs';
  this.options.realm = options.realm || 'urn:example:sp';
  ...
  if (!options.forceAuthn){
    this.options.forceAuthn = false;
  }

  if(!options.validateInResponseTo){
    this.options.validateInResponseTo = false;
  }
...
Samlp.prototype = {
  getSamlRequestUrl: function (req, opts, callback) {
    var self = this;
    var options = xtend(opts || {}, this.options);
    var acsUrl;

    if (!options.identityProviderUrl) {
      throw new Error('identityProviderUrl option is required');
    }

    if (options.path) {
      acsUrl = url.parse(options.path, true);
      if (!acsUrl.protocol || !acsUrl.host) {
        options.path = url.resolve((req.protocol || 'http').concat('://') + 
    ...
    var model = {
      ...
      AssertionConsumerServiceURL: options.path,
    ...
    async.waterfall([
      function(cb) {
        if(self.options.validateInResponseTo) {
          var inResponseTo = xpath.select("/*[local-name()='Response']/@InResponseTo", samlResponse);
          if(inResponseTo){
          ...
          if (inResponseTo) {
            self.cacheProvider.get(inResponseTo, function(err, issueInstant) {
              if (err) { return cb(err); }
              if (!issueInstant) {
                return cb(new Error('SAML Response contains an InResponseTo ID [' + inResponseTo + '] that was not issued in a previous AuthnRequest'));
              }
              return cb();
            })
          }
      ...
      if (!isResponseSigned && !isAssertionSigned) {
        return callback(new Error('SAML Response does not contain a signed response or signed assertion'));
      }
