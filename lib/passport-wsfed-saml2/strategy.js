...
Strategy.prototype.authenticate = function (req, opts) {
  ...
  var options = xtend(this.options, opts);
  ...
  function executeSamlp(req) {
    if (req.body && req.method == 'POST' && req.body.SAMLResponse) {
      ...
      if (!options.identityProviderUrl && options.path) {
        options.identityProviderUrl = url.resolve((req.protocol || 'http').concat('://') + 
      ...
      self._samlp.toSamlRequestUrl(authnRequest, options.identityProviderUrl, options, function(err, url) {
        if (err) return self.error(err);
        self.redirect(url);
      });
    }
