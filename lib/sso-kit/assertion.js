...
var Assertion = function (options) {
...
Assertion.prototype.certToPEM = function (cert) {
...
Assertion.prototype.validateSignature = function (xml, options, callback) {
  var self = this;
...
Assertion.prototype.validateExpiration = function (samlAssertion, version) {
...
Assertion.prototype.validateAudience = function (samlAssertion, audienceUri, version) {
  var audience;
  if (version === '2.0') {
  ...
  return cryptiles.fixedTimeComparison(audience[0].textContent, audienceUri);
};

Assertion.prototype.parseAttributes = function (samlAssertion, version) {
...
Assertion.prototype.validateSamlAssertion = function (samlAssertion, callback) {
  var self = this;
...
Assertion.prototype.parseAssertion = function(samlAssertion, callback) {
  var self = this;
  ...
  if (samlAssertion.getAttribute('MajorVersion') === '1')
    ...
    return callback(new Error('Assertion Assertion version not supported'), null);
  ...
  if (self.options.checkAudience && !self.validateAudience(samlAssertion, self.options.audience, version)) {
    console.log('Audience is invalid. Configured: ' + self.options.audience);
    return callback(new Error('Audience is invalid. Configured: ' + self.options.audience), null);
  }
